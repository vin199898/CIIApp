...
def app():
    model = pickle.load(open('CarbonIntensityIndicator/modelcontainer.sav', 'rb'))
   ...
   # FUNCTION
    def user_report(): 
      ...
      global speed
      global distance
      dwt = st.number_input('Enter Deadweight (Tons)', min_value = 5500, max_value = 230000)
      speed = st.number_input('Enter Service Speed (Knots)', min_value = 10, max_value = 30)
      loa = st.number_input('Enter Length Overall (M)', min_value = 100, max_value = 400)
      beam = st.number_input('Enter Beam (M)', min_value = 5, max_value = 400)
      draft = st.number_input('Enter Draft (M)', min_value = 2, max_value = 15)
      power = st.number_input('Enter Main Engine Output Power (KW)', min_value = 900, max_value = 85000)
